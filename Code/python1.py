	...
	# Define a function that can accept two strings as input and returns 
	# the string with maximum length to the console. 
	
	# If two strings have the same length, then the function should return 
	# both strings separated by a " ".
...
def one(input1, input2):
    ...
    # Return the string that is between the first and last appearance of "bert" 
	# in the given string
...
def four(arg1):
    ...
    # Given a large string that represents a csv, 
	# the structure of each record will be as follows:
    
    # owner,nameOfFile,encrypted?,fileSize
    
    # "Bert,helloWorld.py,True,1447,Bert,strings.py,False,1318,Jeff,dice.py,False,1445"
    
    # For each record, if it is not encrypted,
	#  return the names of the owners of the files in a String Array.
...
def five(input):
    list = [owner, nameOfFile, encrypted?, fileSize]
	if list[2] = "False":
		return list[0]
	else:
		return []
    ...
    # Write a function which returns the boolean True if a string follows the mnemonic, 
	# and returns the boolean False if not.
	...
	# Step through the logic here in order to solve the problem, 
	# you may find help(range) helpful.


def six(input):
	if letter in input = "cei":
		return True
	if letter in input = "ie":
		return True
	else:
	    return False ####
...
def seven(input):
	...
	# Write a function which takes an input (between 1 and 10 inclusive) and 
	# multiplies it by all the numbers before it.
...
def eight(input):
    ...
    # Given a string and a char, returns the position in the String where 
	# the char first appears.
...
def nine(inputString, char):
    ...
    # (represented by the int provided) char of the String supplied
	# is the same as the char supplied.
